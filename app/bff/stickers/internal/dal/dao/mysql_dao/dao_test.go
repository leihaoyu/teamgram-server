...
const testDSN = "root:my_root_secret@tcp(127.0.0.1:3306)/teamgram_stickers?charset=utf8mb4&parseTime=true"
const testBotToken = "8784954709:AAF-N47mj6N_UKn3NbJ2HgPWmryOyFjsJI4"

func setupDB(t *testing.T) *sqlx.DB {
	t.Helper()
	db := sqlx.NewMySQL(&sqlx.Config{DSN: testDSN})
	return db
}

func cleanupSet(t *testing.T, db *sqlx.DB, setId int64) {
	t.Helper()
...
// TestStickerSetsInsertAndSelect tests InsertIgnore + SelectByShortName + SelectBySetId
func TestStickerSetsInsertAndSelect(t *testing.T) {
	db := setupDB(t)
	setsDAO := NewStickerSetsDAO(db)
	ctx := context.Background()

	setId := rand.Int63n(1e15) + 1e14
	defer cleanupSet(t, db, setId)
...
// TestStickerSetsInsertIgnoreDuplicate tests that duplicate InsertIgnore returns rowsAffected=0
func TestStickerSetsInsertIgnoreDuplicate(t *testing.T) {
	db := setupDB(t)
	setsDAO := NewStickerSetsDAO(db)
	ctx := context.Background()
	...
	do2 := &dataobject.StickerSetsDO{
		SetId:       setId + 1, // different setId but same shortName
		AccessHash:  rand.Int63(),
		ShortName:   do.ShortName, // duplicate
	...
	// Cleanup the extra set_id+1 row (shouldn't exist but be safe)
	cleanupSet(t, db, setId+1)
}

// TestStickerSetDocumentsInsertAndSelect tests document InsertIgnore + SelectBySetId
func TestStickerSetDocumentsInsertAndSelect(t *testing.T) {
	db := setupDB(t)
	docsDAO := NewStickerSetDocumentsDAO(db)
	ctx := context.Background()
	...
	// Verify ordering by sticker_index
...
// TestPendingDownloadAndUpdate tests SelectPendingDownloadBySetId + UpdateFileDownloaded
func TestPendingDownloadAndUpdate(t *testing.T) {
	db := setupDB(t)
	docsDAO := NewStickerSetDocumentsDAO(db)
	ctx := context.Background()
...
// TestFullFlowBotAPIToDB fetches a real sticker set from Bot API, writes to DB, reads back and verifies.
func TestFullFlowBotAPIToDB(t *testing.T) {
	db := setupDB(t)
	setsDAO := NewStickerSetsDAO(db)
	docsDAO := NewStickerSetDocumentsDAO(db)
	botClient := dao.NewBotAPIClient(testBotToken)
	ctx := context.Background()
	...
	// 2. Generate IDs and insert sticker set
	setId := rand.Int63n(1e15) + 1e14
	defer cleanupSet(t, db, setId)
	...
	// 3. Build and insert documents (first 5 stickers to keep test fast)
	limit := 5
	...
	for idx := 0; idx < limit; idx++ {
		...
		// Build a Document protobuf like the real handler does
		...
		docDataB64 := base64.StdEncoding.EncodeToString(docData)
	...
	// 4. Read back sticker set
	readSet, err := setsDAO.SelectByShortName(ctx, setDO.ShortName)
	if err != nil || readSet == nil {
		t.Fatalf("SelectByShortName after insert: err=%v, nil=%v", err, readSet == nil)
	}
	...
	if readSet.IsAnimated != setDO.IsAnimated {
		t.Errorf("IsAnimated: got %v, want %v", readSet.IsAnimated, setDO.IsAnimated)
	}
	t.Logf("Read back set: title=%s, count=%d, animated=%v", readSet.Title, readSet.StickerCount, readSet.IsAnimated)

	// 5. Read back and also by SetId
	...
	if readSet2.ShortName != setDO.ShortName {
		t.Errorf("SelectBySetId ShortName: got %s, want %s", readSet2.ShortName, setDO.ShortName)
	}
	t.Logf("SelectBySetId: OK")
	...
	// 7. Deserialize each document_data and verify protobuf roundtrip
	...
	// 9. Mark one as downloaded, verify pending count decreases
	...
	t.Logf("Pending after marking doc %d as downloaded: %d", readDocs[0].DocumentId, len(pending2))
